...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    def isEmpty(x: Any): Boolean = x match {
      case tweetset: Empty => true
      case _               => false
    }
    ...
    if (isEmpty(left) && isEmpty(right)) {
      ...
      getMax(elem, right.mostRetweeted)
    } else if (!isEmpty(left) && isEmpty(right)) {
      getMax(elem, left.mostRetweeted)
    } else {
      getMax(elem, getMax(left.mostRetweeted, right.mostRetweeted))
    }
  }
